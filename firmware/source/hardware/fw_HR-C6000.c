...
*/
	...
	if (slot_state == DMR_STATE_REPEATER_WAKE_4)
	{
		...
		{
			// timecode has not toggled correctly so reset the counter used in the TS state machine
			rxcnt=0;
		}
	}
...
#define END_TICK_TIMEOUT 13
	...
	if (slot_state < DMR_STATE_TX_START_1)
	{
		...
		{
			if ((slot_state == DMR_STATE_RX_1 || slot_state == DMR_STATE_RX_1) && tick_cnt> nonVolatileSettings.endTickTimeOut) // En vez de END_TICK_TIMEOUT
			{
				init_digital_DMR_RX();
				clearActiveDMRID();
				disableAudioAmp(AUDIO_AMP_MODE_RF);
