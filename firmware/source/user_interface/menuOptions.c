...
static const int MAXIMO_END_TICK_RX = 20;  //Maximo valor a introducir


enum OPTIONS_MENU_LIST { OPTIONS_MENU_FACTORY_RESET,OPTIONS_MENU_USE_CALIBRATION,
							OPTIONS_MENU_TX_FREQ_LIMITS,OPTIONS_MENU_KEYPAD_TIMER_LONG, OPTIONS_MENU_KEYPAD_TIMER_REPEAT, OPTIONS_MENU_DMR_MONITOR_CAPTURE_TIMEOUT,
							...
							OPTIONS_MENU_PRIVATE_CALLS,DISPLAY_MENU_END_TICK,OPTIONS_MENU_TXTONE,
...
static void updateScreen(void)
{
	...
	for(int i = -1; i <= 1; i++)
	{
		...
		switch(mNum)
		{
			...
			case DISPLAY_MENU_END_TICK: // 20-02-2020 Evita corte temprano RX
							snprintf(buf, bufferLen, "%s:%d", currentLanguage->endtick , nonVolatileSettings.endTickTimeOut);
							break;
			case OPTIONS_MENU_TXTONE:
				snprintf(buf, bufferLen, "%s:%s", currentLanguage->txtone, (nonVolatileSettings.txTone ? currentLanguage->on : currentLanguage->off));
				break;
...
static void handleEvent(uiEvent_t *ev)
{
	...
	else if (KEYCHECK_PRESS(ev->keys,KEY_RIGHT))
	{
		switch(gMenusCurrentItemIndex)
		{
			...
			case DISPLAY_MENU_END_TICK:  // EB2EAS 20-02-2020

					if (nonVolatileSettings.endTickTimeOut >= 0 && nonVolatileSettings.endTickTimeOut <= MAXIMO_END_TICK_RX  )
					 {

						nonVolatileSettings.endTickTimeOut++;
					}

					if (nonVolatileSettings.endTickTimeOut  > 20)
					{

					nonVolatileSettings.endTickTimeOut= 3;
...
case OPTIONS_MENU_TXTONE:
	...
	else if (KEYCHECK_PRESS(ev->keys,KEY_LEFT))
	{
		...
		switch(gMenusCurrentItemIndex)
		{
			...
			case DISPLAY_MENU_END_TICK:  //20-02-2020
								...
								if (nonVolatileSettings.endTickTimeOut < 3)
								{

									nonVolatileSettings.endTickTimeOut= 20;
...
case OPTIONS_MENU_TXTONE:
				nonVolatileSettings.txTone = false;
				break;
